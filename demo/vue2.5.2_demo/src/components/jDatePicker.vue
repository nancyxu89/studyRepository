<template>
  <j-select ref="j_select" :blurClose="false" placeholder="请输入日期" rightIcon="iconfont icon-rili">
    <div class="date-panel" :class="{'dock-right':align=='right'}">
      ...
      <div class="type-year" v-show="panelType === 'year'">
        <ul class="year-list">
          <li v-for="(item,index) in yearList" v-bind:key="index" v-text="item" :class="{selected: isSelected('year', item), invalid: validateYear(item)}" @click="selectYear(item)">
          </li>
        </ul>
      </div>
      <div class="type-month" v-show="panelType === 'month'">
        <ul class="month-list">
          <li v-for="(item, index) in monthList" v-bind:key="index" :class="{selected: isSelected('month', index), invalid: validateMonth(index)}" @click="selectMonth(index)">
            {{item | month(language)}}
          </li>
        </ul>
      </div>
      <div class="type-date" v-show="panelType === 'date'">
        <ul class="weeks">
          <li v-for="(item,index) in weekList" v-bind:key="index">{{item | week(language)}}</li>
        </ul>
        <ul class="date-list">
          <li v-for="(item, index) in dateList" v-bind:key="index" :class="{preMonth: item.previousMonth, nextMonth: item.nextMonth,
...
export default {
  ...
  methods: {
    togglePanel(state) {
      this.$refs.j_select.showPanel = state
      this.panelState = state
      this.rangeStart = false
    },
    setValue(value) {
      if ((this.curValue || '') === (value || '')) return
      this.$refs.j_select.text = value
      this.curValue = value
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.minChange(this.min)
      this.maxChange(this.max)
